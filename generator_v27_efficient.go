...
// GenerateStringArtV27Efficient implements v3.3.0+ with performance optimization:
// 1. 4x Birsak 2018 supersampling (16 gray levels - good balance)
// 2. Fast MSE-based line scoring with importance weighting
// 3. SSIM validation every 200 lines (not per-line)
// 4. Heuristic-based line removal (contribution tracking)
// 5. Adaptive brightness targeting (aim for mean ~102)
func GenerateStringArtV27Efficient(img *image.Gray, edgeMap *image.Gray, config *Config) ([]Line, [][]float64) {
	bounds := img.Bounds()
	width, height := bounds.Dx(), bounds.Dy()

	// Use 4x supersampling for 16 gray levels (good balance of quality vs speed)
	supersample := 4
	superWidth := width * supersample
	superHeight := height * supersample

	fmt.Printf("=== String Art Generator v27.0 Efficient ===\n")
	fmt.Printf("Base Resolution: %dx%d\n", width, height)
	fmt.Printf("Super Resolution: %dx%d (4x supersampling = 16 gray levels)\n", superWidth, superHeight)
	...
	importance := createV27ImportanceMap(img, edgeMap, width, height)

	// Pre-compute line pixels at supersampled resolution
	fmt.Println("Pre-computing supersampled line pixels...")
	linePixels := precomputeLinePixelsV27(pins, superWidth, superHeight, config.MinDistance*supersample)
	fmt.Printf("Pre-computed %d valid line segments\n", len(linePixels))

	lines := make([]Line, 0, config.NumLines)
	currentPin := 0
	usedLines := make(map[[2]int]int)
	lineContributions := make(map[int]float64) // Track each line's contribution

	// Calibrated alpha for mobile SVG - tuned for 4x supersampling
	// With 4x, we need alpha ~0.75 to match mobile SVG rendering
	// Lower alpha = more aggressive darkening per line
	alpha := 0.75

	// Adaptive weight targeting mean brightness ~102
	// Start with base weight and adjust based on progress
	baseWeight := float64(config.LineWeight)
	fmt.Printf("Base weight: %.1f, Alpha: %.2f\n", baseWeight, alpha)

	// Phase 1: Fast greedy addition with MSE-based scoring
	fmt.Println("\n--- Phase 1: Fast MSE Greedy Addition (4x Supersampling) ---")
	...
	for i := 0; i < config.NumLines; i++ {
		...
		bestLine := findBestLineV27Fast(currentPin, pins, superCanvas, target, importance,
		...
		applyLineToSuperCanvasV27(superCanvas, linePixels[[2]int{bestLine.From, bestLine.To}], adaptiveWeight, alpha)

		// Track line contribution
		lineContributions[len(lines)] = bestLine.Score
		...
		if (i+1)%200 == 0 {
			...
			meanBrightness := computeMeanBrightnessV27(superCanvas, supersample)
			localSSIM := computeLocalSSIMV27(superCanvas, target, supersample)
	...
	// Phase 2: Heuristic-based line removal
	fmt.Println("\n--- Phase 2: Heuristic Line Removal ---")
	initialSSIM := computeLocalSSIMV27(superCanvas, target, supersample)
	initialBrightness := computeMeanBrightnessV27(superCanvas, supersample)
	fmt.Printf("Initial SSIM: %.4f, Brightness: %.1f\n", initialSSIM, initialBrightness)

	removed := 0
	maxRemovals := len(lines) / 8 // Remove up to 12.5% of lines

	// Build removal candidates based on contribution scores
	...
	// Find lines with low contribution (bottom 20%)
	for i, contrib := range lineContributions {
		if i < len(lines) {
			candidates = append(candidates, RemovalCandidate{index: i, contribution: contrib})
		}
	}
	...
	for i := 0; i < len(candidates); i++ {
		for j := i + 1; j < len(candidates); j++ {
			if candidates[j].contribution < candidates[i].contribution {
				candidates[i], candidates[j] = candidates[j], candidates[i]
			}
		}
	}
	
	fmt.Printf("Evaluating %d removal candidates...\n", len(candidates))
	
	// Remove low-contribution lines
	removedIndices := make(map[int]bool)
	checkInterval := 20 // Check SSIM every 20 removals
	...
	for idx, candidate := range candidates {
		...
		removeLineFromSuperCanvasV27(superCanvas, linePixels[[2]int{line.From, line.To}], baseWeight, alpha)
		
		// Check SSIM periodically
		shouldKeepRemoved := true
		if idx%checkInterval == 0 || removed == 0 {
			newSSIM := computeLocalSSIMV27(superCanvas, target, supersample)
			newBrightness := computeMeanBrightnessV27(superCanvas, supersample)
			
			// Keep removed if SSIM doesn't drop significantly and brightness improves
			brightnessImprovement := math.Abs(newBrightness-102.0) < math.Abs(initialBrightness-102.0)
			ssimOK := newSSIM >= initialSSIM-0.001
			
			shouldKeepRemoved = ssimOK && (brightnessImprovement || newSSIM > initialSSIM)
			
			if shouldKeepRemoved {
				initialSSIM = newSSIM
				initialBrightness = newBrightness
				fmt.Printf("Removed %d lines (SSIM: %.4f, Brightness: %.1f)\n", removed+1, newSSIM, newBrightness)
			}
		}
		...
		if shouldKeepRemoved {
			...
			// Put it back
			applyLineToSuperCanvasV27(superCanvas, linePixels[[2]int{line.From, line.To}], baseWeight, alpha)
		}
	}
	...
	canvas := downsampleCanvasV27(superCanvas, supersample)

	finalSSIM := computeLocalSSIMV27(superCanvas, target, supersample)
	finalBrightness := computeMeanBrightnessV27(superCanvas, supersample)
	fmt.Printf("\n=== Final Results ===\n")
...
// createV27ImportanceMap creates importance map with enhanced face detection
func createV27ImportanceMap(img *image.Gray, edgeMap *image.Gray, width, height int) [][]float64 {
	...
	// Detect face region
	faceRegion := detectFaceRegionV27(img, width, height)
...
// detectFaceRegionV27 detects likely face region
func detectFaceRegionV27(img *image.Gray, width, height int) [][]bool {
	...
	// Find dark regions (face features)
...
// precomputeLinePixelsV27 precomputes all valid line segments
func precomputeLinePixelsV27(pins []Pin, width, height, minDistance int) map[[2]int][]SuperPixel {
	...
	for from := 0; from < numPins; from++ {
		for to := from + 1; to < numPins; to++ {
			...
			pixels := bresenhamSuperV27(pins[from], pins[to], width, height)
			if len(pixels) > 0 {
...
// bresenhamSuperV27 computes line pixels using Bresenham algorithm
func bresenhamSuperV27(p1, p2 Pin, width, height int) []SuperPixel {
...
// findBestLineV27Fast finds the best line using fast MSE-based scoring
func findBestLineV27Fast(currentPin int, pins []Pin, superCanvas, target [][]float64,
	...
	for w := 0; w < config.Workers; w++ {
		...
		go func(workerID int) {
			...
			for to := start; to < end; to++ {
				...
				// Calculate fast MSE-based score
				score := calculateFastMSEScoreV27(pixels, superCanvas, target, importance, weight, alpha, supersample)
...
// calculateFastMSEScoreV27 calculates fast MSE-based improvement score
func calculateFastMSEScoreV27(pixels []SuperPixel, superCanvas, target [][]float64,
	...
	// Sample pixels for efficiency (every 2nd pixel in supersampled space)
	sampleStep := 2
	totalScore := 0.0
	sampleCount := 0
...
// applyLineToSuperCanvasV27 applies a line to the supersampled canvas
func applyLineToSuperCanvasV27(canvas [][]float64, pixels []SuperPixel, weight, alpha float64) {
...
// removeLineFromSuperCanvasV27 removes a line from the supersampled canvas
func removeLineFromSuperCanvasV27(canvas [][]float64, pixels []SuperPixel, weight, alpha float64) {
	for _, p := range pixels {
		canvas[p.Y][p.X] += weight * alpha
...
// computeLocalSSIMV27 computes local windowed SSIM (matching Python validator)
func computeLocalSSIMV27(superCanvas, target [][]float64, supersample int) float64 {
...
// computeMeanBrightnessV27 computes mean brightness of downsampled canvas
func computeMeanBrightnessV27(superCanvas [][]float64, supersample int) float64 {
...
// downsampleCanvasV27 downsamples supersampled canvas to base resolution
func downsampleCanvasV27(superCanvas [][]float64, supersample int) [][]float64 {
	superHeight := len(superCanvas)
	superWidth := len(superCanvas[0])
	height := superHeight / supersample
	width := superWidth / supersample
