...
// GenerateStringArtV18Birsak implements the Birsak 2018 supersampled rendering approach
// with proper calibration for mobile SVG rendering and enhanced optimization
func GenerateStringArtV18Birsak(img *image.Gray, edgeMap *image.Gray, config *Config) ([]Line, [][]float64) {
	bounds := img.Bounds()
	width, height := bounds.Dx(), bounds.Dy()

	// Use 8x supersampling as per Birsak 2018 paper for high quality
	supersample := 8
	superWidth := width * supersample
	superHeight := height * supersample

	fmt.Printf("=== String Art Generator v18.0 Birsak 2018 ===\n")
	...
	importance := createV18ImportanceMap(img, edgeMap, width, height)

	// Pre-compute line pixels at supersampled resolution
	fmt.Println("Pre-computing supersampled line pixels...")
	linePixels := precomputeLinePixelsV18(pins, superWidth, superHeight, config.MinDistance*supersample)
	...
	// Calibrated alpha for mobile SVG matching - tuned based on baseline analysis
	// The baseline has mean brightness 111, we want to match that
	alpha := 0.05 // Lower alpha for 8x supersampling

	// Phase 1: Greedy line addition
	fmt.Println("\n--- Phase 1: Greedy Line Addition ---")
	for i := 0; i < config.NumLines; i++ {
		// Adaptive line weight - start higher, reduce gradually
		progress := float64(i) / float64(config.NumLines)
		adaptiveWeight := float64(config.LineWeight) * (1.0 + 0.5*(1.0-progress))
		if adaptiveWeight < 20 {
			adaptiveWeight = 20
		}

		bestLine := findBestLineV18(currentPin, pins, superCanvas, target, importance,
		...
		applyLineToSuperCanvasV18(superCanvas, linePixels[[2]int{bestLine.From, bestLine.To}], adaptiveWeight, alpha)
		...
		if (i+1)%200 == 0 {
			...
			currentSSIM := computeCurrentSSIMV18(superCanvas, target, supersample)
			meanBrightness := computeMeanBrightness(superCanvas, supersample)
	...
	fmt.Println("\n--- Phase 2: Add/Remove Optimization ---")
	initialSSIM := computeCurrentSSIMV18(superCanvas, target, supersample)
	fmt.Printf("Initial SSIM: %.4f\n", initialSSIM)

	// Try removing lines that hurt quality
	removed := 0
	improved := true
	for improved && removed < 200 {
		improved = false
		bestRemovalIndex := -1
		bestSSIMAfterRemoval := initialSSIM

		// Test removing each line
		for i := len(lines) - 1; i >= 0 && i >= len(lines)-50; i-- { // Only test recent lines
			line := lines[i]
			
			// Remove line temporarily
			removeLineFromSuperCanvasV18(superCanvas, linePixels[[2]int{line.From, line.To}], float64(config.LineWeight), alpha)
			
			// Check if quality improved
			newSSIM := computeCurrentSSIMV18(superCanvas, target, supersample)
			...
			// Put it back for now
			applyLineToSuperCanvasV18(superCanvas, linePixels[[2]int{line.From, line.To}], float64(config.LineWeight), alpha)
		}

		// If we found an improvement, remove the best line
		if bestRemovalIndex >= 0 && bestSSIMAfterRemoval > initialSSIM + 0.001 {
	...
	canvas := downsampleCanvasV18(superCanvas, supersample)

	finalSSIM := computeCurrentSSIMV18(superCanvas, target, supersample)
	finalBrightness := computeMeanBrightness(superCanvas, supersample)
	fmt.Printf("Final SSIM: %.4f\n", finalSSIM)
	fmt.Printf("Final brightness: %.1f (target: ~111)\n", finalBrightness)
	fmt.Printf("Total lines: %d\n", len(lines))

	return lines, canvas
}

// createV18ImportanceMap creates an importance map focused on facial features
func createV18ImportanceMap(img *image.Gray, edgeMap *image.Gray, width, height int) [][]float64 {
	...
	// Enhanced face detection for cat
	centerX, centerY := width/2, height/2
	faceRadius := int(float64(math.Min(float64(width), float64(height))) * 0.3)

	// Eye regions (cats have eyes in upper-middle area)
...
// precomputeLinePixelsV18 precomputes all valid line segments
func precomputeLinePixelsV18(pins []Pin, width, height, minDistance int) map[[2]int][]AntiAliasedPixel {
	numPins := len(pins)
...
// findBestLineV18 finds the best line using perceptual scoring
func findBestLineV18(currentPin int, pins []Pin, canvas [][]float64, target [][]float64,
	...
	for nextPin := 0; nextPin < len(pins); nextPin++ {
		...
		score := computeLinePerceptualScore(canvas, target, importance, pixels, weight, supersample, alpha)
...
// computeLinePerceptualScore computes perceptual score for a potential line
func computeLinePerceptualScore(canvas [][]float64, target [][]float64, importance [][]float64,
...
// applyLineToSuperCanvasV18 applies a line to the supersampled canvas
func applyLineToSuperCanvasV18(canvas [][]float64, pixels []AntiAliasedPixel, weight, alpha float64) {
	for _, pixel := range pixels {
		if pixel.Y >= 0 && pixel.Y < len(canvas) && pixel.X >= 0 && pixel.X < len(canvas[0]) {
...
// removeLineFromSuperCanvasV18 removes a line from the supersampled canvas
func removeLineFromSuperCanvasV18(canvas [][]float64, pixels []AntiAliasedPixel, weight, alpha float64) {
	for _, pixel := range pixels {
		if pixel.Y >= 0 && pixel.Y < len(canvas) && pixel.X >= 0 && pixel.X < len(canvas[0]) {
...
// computeCurrentSSIMV18 computes SSIM between current canvas and target
func computeCurrentSSIMV18(superCanvas [][]float64, target [][]float64, supersample int) float64 {
	...
	return computeSSIMV18(img1, img2, width, height)
}

// computeMeanBrightness computes mean brightness of supersampled canvas
func computeMeanBrightness(superCanvas [][]float64, supersample int) float64 {
	canvas := downsampleCanvasV18(superCanvas, supersample)
...
// downsampleCanvasV18 downsamples supersampled canvas to base resolution
func downsampleCanvasV18(superCanvas [][]float64, supersample int) [][]float64 {
...
// computeSSIMV18 computes Structural Similarity Index between two images
func computeSSIMV18(img1, img2 []float64, width, height int) float64 {
